import EmblaCarousel from "//botari.co/cdn/shop/t/2/assets/m-carousel.js?v=7956072137199587391776263156"
import ClassNames from "//botari.co/cdn/shop/t/2/assets/h-carousel-cn.js?v=74413230910519084281776263156"
    ...
    customElements.define('coretex-slideshow', class CoretexSlideshow extends HTMLElement {
        ...
        async connectedCallback() {
            ...
            if (this.hasAttribute('autoplay')) {
                ...
                const { default: Autoplay } = await import("//botari.co/cdn/shop/t/2/assets/h-carousel-ap.js?v=151989844542395062071776263156"); // conditionally load plugin w/ dynamic imports
            ...
            if (this.hasAttribute('auto-height')) {
                const { default: AutoHeight } = await import("//botari.co/cdn/shop/t/2/assets/h-carousel-ah.js?v=44275636832617084041776263156"); // conditionally load plugin w/ dynamic imports
                plugins.push(AutoHeight({destroyHeight: 'auto'}));
            }

            // Check for slider-fade attribute
            if (this.hasAttribute('fade')) {
                const { default: Fade } = await import("//botari.co/cdn/shop/t/2/assets/h-carousel-fa.js?v=36576538050214963121776263156"); // conditionally load plugin w/ dynamic imports
                plugins.push(Fade());
            }
            ...
            this.videoPlayers = Array.from(this.querySelectorAll('video-player'));
        ...
        init() {
            ...
            this.embla.on('select', this.toggleVideoPlayback.bind(this)); // Add this line
            this.embla.on('init', this.toggleVideoPlayback.bind(this));   // Add this line
            this.a11y();
            this.buttons();
            this.dots();
            this.numbers();
        ...
        toggleVideoPlayback() {
            if (!this.videoPlayers.length) return;

            const selectedIndex = this.embla.selectedScrollSnap();

            this.videoPlayers.forEach((player, index) => {
                const video = player.querySelector('video');
                const controller = player.querySelector('.controller');

                if (video) {
                    if (index === selectedIndex) {
                        // Play video for the selected slide if it has autoplay
                        if (video.autoplay && video.paused) {
                            video.play().then(() => {
                                player.classList.add('playing');
                                if (controller) controller.setAttribute('aria-label', 'Pause');
                            }).catch(error => console.error("Video play failed:", error));
                        }
                    } else {
                        // Pause video for non-selected slides
                        if (!video.paused) {
                            video.pause();
                            player.classList.remove('playing');
                            if (controller) controller.setAttribute('aria-label', 'Play');
                        }
                    }
                }
            });
